...
export default function Home() {
  ...
  const calc = () => {
    ...
    setResult({fc,cya,eff:+eff.toFixed(3),lock:+lock.toFixed(1),status:lock>70?"danger":lock>50?"warning":"optimal"});
  };
  return (
    <main className="max-w-[720px] mx-auto px-6 py-16">
      <header className="flex items-center justify-between mb-20 pb-6 border-b border-cream-border"><div className="font-serif text-xl font-bold tracking-tight">AQUARIGHT</div></header>
      <section className="mb-20">
        <h1 className="font-serif text-4xl md:text-5xl leading-tight font-bold text-ink mb-6">FCI over 150ppm and your chlorine demand doubles overnight.</h1>
        <p className="text-lg text-ink-muted leading-relaxed mb-8">AQUARIGHT gives pool professionals water chemistry calculations — FCI lockup, CSI saturation index, chemical dose — without memorizing the Taylor test kit manual.</p>
          ...
          {result && (
            <div className="space-y-3">
              <div className="text-center">
                ...
                <div className={`text-sm px-3 py-1 rounded-full inline-block mt-2 ${result.status==="optimal"?"bg-green-100 text-green-700":result.status==="warning"?"bg-yellow-100 text-yellow-700":"bg-red-100 text-red-700"}`}>
                  {result.status==="optimal"?"Optimal":result.status==="warning"?"Warning":"DANGER"}
                </div>
              </div>
      ...
      <footer className="border-t border-cream-border pt-8 flex justify-between text-xs text-ink-muted"><span className="font-serif font-bold text-ink">AQUARIGHT</span><span>Built by KRYZL19</span></footer>
    </main>
  );
}
